...
const VehiclesScreen = ({ navigation }) => {
  ...
  const fetchVehicles = useCallback(async () => {
    ...
    try {
      ...
      const fetchedVehicles = await api.getVehiclesByDriver(driverId);
  ...
  const handleToggleActive = async (vehicleId, currentStatus) => {
    try {
      await api.updateVehicleStatus(vehicleId, !currentStatus);
  ...
  return (
    <Box flex={1} bg="white">
      ...
      <Box position="absolute" bottom={20} right={8}>
        <TouchableOpacity
      ...
      <SimpleTabBar navigation={navigation} currentTab="Profile" />
    </Box>
  );
};
